...
import { Trophy, Share2, ExternalLink, Star, Crown, Loader2, RefreshCw, ArrowLeft, Lock, MonitorPlay, Play } from "lucide-react";
import { Confetti } from "@/components/ui/confetti";
import { toast } from "sonner";
import Link from "next/link";
import { getAwardsData } from "../data-fetcher"; // Adjusted import
...
export default function BandlabAwardsPage() {
    ...
    return (
        <main className="min-h-screen bg-background text-foreground selection:bg-yellow-500/30 font-sans overflow-x-hidden transition-colors duration-300">
            ...
            {/* Back Button -> Goes to Hub now */}
            <div className="absolute top-6 left-6 z-50">
                <Link href="/events" className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors bg-white/50 dark:bg-black/50 backdrop-blur-md px-4 py-2 rounded-full border border-black/10 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/10">
            ...
            <div className="relative min-h-[80vh] flex flex-col items-center justify-center p-6 text-center border-b border-border">
                ...
                >
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-yellow-500/30 bg-yellow-500/10 text-yellow-600 dark:text-yellow-500 text-sm font-bold uppercase tracking-widest mb-4 backdrop-blur-md">
                        <Star className="w-4 h-4 fill-yellow-600 dark:fill-yellow-500" /> Current Event
                    </div>
                    ...
                    <p className="text-xl md:text-2xl text-muted-foreground max-w-2xl font-light">
                        Celebrating the icons of our community.
                    </p>
                    ...
                    <div className="flex flex-col md:flex-row gap-4 mt-8">
                        ...
                        <Link
                            href="/events/bandlab2025/teaser"
                            className="group px-8 py-4 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-full font-bold text-foreground hover:bg-black/10 dark:hover:bg-white/10 transition-all flex items-center justify-center gap-3"
                        >
